...
@Component({
  selector: 'app-player-two-board',
  templateUrl: './player-two-board.component.html',
  styleUrls: ['./player-two-board.component.css']
})
export class PlayerTwoBoardComponent {
  ...
  initYclick: number = 0;
  ////constructor
  constructor(public renderer: Renderer2, private webSocketService: MoveService, private router: Router, private userServe: UserServicesService) {
    this.msg = 'msg'
    this.chessPieces = [];
    this.curPiece = ''
    this.item = [];
  ...
  ngOnInit() {
    ...
    this.webSocketService.socket$.subscribe(
      (message) => {
        ...
        if (msg[0].type == 'move' && msg[0].gameId == this.gameId) {
          this.chessPieces = JSON.parse(message[0].msg)
          this.clearBoard();
          this.setBoard();
          this.waiting = false;
        }
        if (msg[0].type == 'start' && msg[0].gameId == this.gameId) {
          alert('start' + this.router.url)
          this.ShowBoard();
          this.waiting = false;
        }
    ...
    let fl = 0;
    console.log('this is hiiasjdlkashdljk')
