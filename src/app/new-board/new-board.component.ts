...
export class NewBoardComponent {
  ...
  ////constructor
  constructor(public renderer: Renderer2, private webSocketService: MoveService) {
  ...
  ngOnInit() {
    ...
    this.webSocketService.socket$.subscribe(
      (message) => {
        ...
        ///imp to omit the "" from the string that we are passing 
        console.log(message)

        if (message === '') {
          console.log('this is message null')
        } else {
          this.fenConverter(message.replace('"', ''))
    ...
    for (let i = 1; i <= 8; i++) {
      for (let j = 1; j <= 8; j++) {
        if (this.setPiece(i, j) == true) {
          const a = this.convertToString(j, i)
          const element = document.getElementById(a);
          let svgHtml = 'pawn'
          ///Black piece
          let piece = this.findPiece(i, j)
          if (piece === 'BP') {
  ...
  fenConverter(fenstr: string) {
    ...
    console.log('this are chess pieces')
    console.log(this.chessPieces)
    this.setBoard();
  }
  /////fen ends
